...
public final class HyperspaceHooks {
    private static volatile AgentConfig cfg;
    ...
    static synchronized void init(AgentConfig c){
        cfg=c;
        if(c!=null && c.statsIntervalNanos>0L)startStatsThread();
    }
    private static void startStatsThread(){
        Thread running=statsThread.get();
        if(running!=null && running.isAlive())return;
        try{
            Thread thread=new Thread(HyperspaceHooks::statsLoop,"StarsectorHyperspaceOptimizer-Stats");
            ...
            try{AgentLog.error("hyperspace stats logger could not start",ex);}catch(Throwable ignored){}
        }
    }
    ...
    @SuppressWarnings({"rawtypes","unchecked"}) public static boolean removeAllAndRelease(Collection receiver,Collection removals){
        ...
        try{
            if(cfg!=null && cfg.optimizedRemoveAll && removals!=null && receiver!=null && removals.size()>=cfg.removeAllThreshold && receiver.size()>=cfg.removeAllThreshold*2){
    ...
    static long pooledRandomApprox(){
    ...
    private static void statsLoop(){
        ...
        try{
            while(true){
                AgentConfig c=cfg;if(c==null||c.statsIntervalNanos<=0L)return;
                long millis=Math.max(1L,c.statsIntervalNanos/1_000_000L);
                Thread.sleep(millis);
                try{AgentLog.info(statsSnapshot());}
                catch(Throwable ex){AgentLog.error("hyperspace stats logger failed",ex);}
            }
