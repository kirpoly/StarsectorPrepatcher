...
причины/остаточного риска в отчёте соответствующего выпуска из [`releases/`](releases/0.9.1.md).
...
| `patch.directMarketObservation` | mod classes в system/child loaders: ARG/CONST/derived amount, несколько calls на method, exception, reflection/MethodHandle и nested direct call | `call-sites.csv`, `observations.csv`, `stacks.csv`, `unknown-stacks.csv`, `summary.csv`, `prepatcher.log`; JFR overhead A/B | точные multiplicity/amount/order/thread/exception сохраняются; direct call остаётся synchronous; call-site metadata указывает JAR/class/method; reflective/untransformed вход попадает в bounded unknown samples; observer failure не подавляет original call |
